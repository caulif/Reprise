...
export function projectPackEntries(event: EventEnvelope): TimelineEntry[] {
  const productId = event.type.startsWith('claude-code.') ? 'claude-code' : 'codex';
  return packActivity(findProductPack(productId)).translate(event).flatMap((item, index) => projectTimelineEvent({
    ...event,
