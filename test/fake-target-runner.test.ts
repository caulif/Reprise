...
import { fakeActivityTranslator } from './fixtures/fake-pack/activity.js';
...
test('CandidateRuntimeEvent schema requires sessionId, sequence, and evidence refs', () => {
  const event = {
    eventId: 'evt-1',
    sequence: 1,
  ...
  assert.equal(Value.Check(CandidateRuntimeEventSchema, event), true);
  assert.equal(Value.Check(CandidateRuntimeEventTypeSchema, event.type), true);
  assert.equal(Value.Check(CandidateRuntimeEventSchema, { ...event, sequence: 0 }), false);
});
