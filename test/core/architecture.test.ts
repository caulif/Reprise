...
test('internal agents share workspace tools without read_observation', async () => {
  ...
  assert.match(runModel, /allowShell:\s*true/);
  assert.doesNotMatch(runModel, /input\.allowShell/);
  assert.match(runModel, /SOURCE_MOUNT/);
  assert.match(runModel, /staging\.sourceRoot/);
  ...
  assert.doesNotMatch(runFinalize, /applyTaskReadinessGate/);
  assert.doesNotMatch(runFinalize, /taskReadinessBlocksPublication/);
});
...
test('Recovery production path does not reintroduce candidate selection or three-state envelopes', async () => {
  ...
  const envelopeSchema = await readFile(join(SRC, 'core/schemas/recovery.ts'), 'utf8');
  assert.match(envelopeSchema, /Type\.Literal\("ready"\)/);
  assert.match(envelopeSchema, /Type\.Literal\("blocked"\)/);
  assert.match(envelopeSchema, /RecoverySummarySchema/);
  assert.match(envelopeSchema, /maxLength: 240/);
  const runModel = await readFile(join(SRC, 'application/recovery/run-model.ts'), 'utf8');
  ...
  assert.match(beginRecovery, /workspaceSeed = 'sparse'/);
  assert.match(beginRecovery, /inspected\.mode !== 'canonical'/);
  ...
  assert.doesNotMatch(validateBody, /recovered_partial|insufficient_evidence|extraNotes|current_state_fallback/);
  assert.match(validateBody, /summary: result\.summary/);
  const inspectBody = provider.match(/async inspectBaseline[\s\S]*?\n {2}async captureRecoveryCheckpoint/)?.[0] ?? "";
  assert.doesNotMatch(inspectBody, /workspace-budget/);
  assert.doesNotMatch(inspectBody, /runnable: budget\.blockedReasons/);
  const lockSource = await readFile(join(SRC, 'environment/source-write-lock.ts'), 'utf8');
  assert.match(lockSource, /\(WD,AD,DC\)/);
  assert.doesNotMatch(lockSource, /\(OI\)\(CI\)\(WD,AD,DC,D\)/);
  const fsSource = await readFile(join(SRC, 'environment/local-workspace-fs.ts'), 'utf8');
  const matchFn = fsSource.match(/export function baselineMatchFromRecoveryStatus[\s\S]*?\n}/)?.[0] ?? "";
  assert.match(matchFn, /return "observational"/);
  assert.doesNotMatch(matchFn, /current_state_fallback/);
  const userStatus = await readFile(join(SRC, 'application/recovery/user-status.ts'), 'utf8');
  ...
  assert.doesNotMatch(run, /tryHostCheckpointRecovery|completeHostCheckpointRecovery/);
  assert.doesNotMatch(run, /isRepo/);
  assert.doesNotMatch(runModel, /isRepo/);
  assert.match(runModel, /resetRecoveryWorkspace/);
  assert.match(runModel, /releasePreparation/);
  assert.match(runModel, /workspace_damaged/);
  assert.match(runModel, /ProcessBoundaryError/);
  const readiness = await readFile(join(SRC, 'application/recovery/readiness.ts'), 'utf8');
  ...
  await assert.rejects(stat(join(SRC, 'application/recovery/checkpoint.ts')));
...
test('role write policy stays on application owners without a shared Verifier', async () => {
  ...
  assert.doesNotMatch(inspection, /historicalUserFollowups/);
  const controllerAgent = await readFile(join(SRC, 'agents/controller-agent.ts'), 'utf8');
  assert.doesNotMatch(controllerAgent, /按顺序读取全部/);
  assert.doesNotMatch(comparisonAgent, /按索引顺序读取全部/);
  const comparisonBriefing = await readFile(join(SRC, 'application/comparison-briefing.ts'), 'utf8');
  assert.doesNotMatch(comparisonBriefing, /every user turn in index order/);
});
