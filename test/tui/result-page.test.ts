...
test('synthetic fixture covers cancelled and insufficient_evidence comparison independently of candidate success', () => {
  ...
  // T08: cancelled paints as "Comparison cancelled" (not complete); fixture must still carry cancelled status.
  const cancelledPaint = renderResult(theme, 120, cancelledFixture as never).join('\n');
  assert.match(cancelledPaint, /apparently_completed|completed\.controller_satisfied|Controller judged complete/);
