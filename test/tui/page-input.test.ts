...
test('inspection, preflight, confirm, running, result, and error dispatch the operator keys', () => {
  ...
  assert.equal(dispatchResultKeys('c', { comparePending: true }), undefined);
  ...
  assert.equal(dispatchResultKeys('h', { artifacts: { historyFinal: true } }), undefined);
  assert.equal(dispatchResultKeys('f', { artifacts: { candidateFinal: true } }), undefined);
  assert.equal(dispatchResultKeys('t', { artifacts: { trace: true } }), undefined);
  assert.equal(dispatchResultKeys('w', { artifacts: { replica: true } }), undefined);
