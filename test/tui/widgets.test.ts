...
function targetEntry(title: string, detail: string, extra: Partial<TimelineEntry> = {}): TimelineEntry {
  return {
    sequence: extra.sequence ?? 1,
    occurredAt: extra.occurredAt ?? '2026-08-11T00:10:02.000Z',
    source: 'TARGET',
    title,
    detail,
    ...extra,
...
test('detail pane indents command output so it does not stick to the frame', () => {
  const theme = createTheme(120, false);
  const entry = targetEntry('pwsh · Get-Content', '$ Get-Content\n| Sandbox blocked a path outside the isolated workspace.\n| Windows could not apply deny-read ACLs.\nexit -1', {
    sequence: 29,
    occurredAt: '2026-08-13T14:17:28.260Z',
    level: 'warning',
    original: '"C:\\\\Program Files\\\\PowerShell\\\\7\\\\pwsh.exe" -Command Get-Content',
  });
  const lines = renderTimeline(theme, 120, {
    entries: [entry],
    selected: 0, filter: 'ALL', following: true, cancelling: false,
...
test('command detail paints a one-line invocation plus indented output', () => {
  const theme = createTheme(120, false);
  const entry = targetEntry('pwsh · Get-ChildItem', '$ Get-ChildItem | Format-Table Mode,Length,LastWriteTime,Name\n| Mode  Length LastWriteTime         Name\n| ----  ------ -------------         ----\n| -a---   1200 8/13/2026 12:00:00 AM  file-1.txt\nexit 0 · 476ms', {
    sequence: 5,
    occurredAt: '2026-08-11T00:10:01.500Z',
    original: '"C:\\\\Program Files\\\\PowerShell\\\\7\\\\pwsh.exe" -Command "Get-ChildItem | Format-Table Mode,Length,LastWriteTime,Name"',
  });
