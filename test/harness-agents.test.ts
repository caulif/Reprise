...
test('Harness agent factory shares one Pi session caller and persisted model choice', async () => {
  ...
  const decision = await agents.controller.decide({ runId: 'run-1', runState: 'awaiting_controller', task: { initialInput: { id: 'input-1', role: 'user', text: 'Fix it.' }, baseline: { status: 'available', artifactRefs: [], evidenceRefs: [] }, privacy: { allowModelText: true, allowBinary: false, redactions: [] } }, current: { summary: 'Settled.', evidenceRefs: [] }, trajectory: { summary: 'One turn.', evidenceRefs: [] }, budget: { decisionsUsed: 1, decisionsLimit: 2 } });
  assert.equal(decision.status, 'completed');
  if (decision.status === 'completed') assert.equal(decision.value.type, 'done');
  assert.equal(calls, 1);
  assert.deepEqual(agents.config, { providerId: 'openai-codex', requestedModel: 'gpt-5.6-terra', budget: { callTimeoutMs: 90_000, maxStructuredRepairAttempts: 1 } });
});
