...
import type { ProductPack } from '../../src/products/contract.js';
...
test('activity other-kind share stays below the contract threshold', () => {
  ...
  const fakeFacts = fakeProductPack.projection.inspectRunFacts(fakeEvents);
  assert.equal(fakeFacts.finalMessage, 'world');
  ...
  const claudeFacts = claudeCodeProductPack.projection.inspectRunFacts(claudeEvents);
  assert.equal(claudeFacts.finalMessage, 'working');
  assert.deepEqual(claudeFacts.commands, ['echo']);
});
