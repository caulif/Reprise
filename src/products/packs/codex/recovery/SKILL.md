...
2. Use shell_exec for remaining bounded work; cwd is already the writable copy. Copy from the directory named by REPRISE_SOURCE_MOUNT when a source file is needed; the OS denies writes back to that path and the Host verifies the source fingerprint. The local developer environment may be used for installs and builds; credentials, the user's real directory, and global Git config stay out of bounds.
