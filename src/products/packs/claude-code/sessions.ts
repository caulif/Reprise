...
type ClaudeSummaryState = DiscoverySummaryState;
...
async function summarizeClaudeSessionHead(entry: SessionFileEntry, signal: AbortSignal | undefined): Promise<SessionSummary> {
  ...
  const state: ClaudeSummaryState = emptyDiscoverySummaryState();
  ...
  requireDiscoveredSessionId(state.sessionId, 'Claude', 'head');
...
async function summarizeClaudeSession(entry: SessionFileEntry, signal: AbortSignal | undefined): Promise<SessionSummary> {
  ...
  const fileId = basename(sourcePath, '.jsonl');
  state.sessionId ??= SAFE_ID.test(fileId) ? fileId : undefined;
  requireDiscoveredSessionId(state.sessionId, 'Claude', 'full');
...
function claudeSummaryFromState(entry: SessionFileEntry, sourcePath: string, state: ClaudeSummaryState, partial: boolean): SessionSummary {
  ...
  return sessionSummaryFromDiscovery({
    productId: PRODUCT_ID,
    entry,
    sourcePath,
    state,
    ...
    ...(partial
      ? { recoveryDiagnostics: [{ code: 'summary-window', message: 'List used a bounded head; full inspect is required.' }] }
      : readiness === 'no-user-input'
        ? { recoveryDiagnostics: [{ code: 'no-user-input', message: 'Complete discovery summary found no eligible user message.' }] }
        : {}),
  });
}
