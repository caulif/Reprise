...
export function comparisonReportTool(reportRoot: string): AgentToolDefinition {
  return { name: 'write_comparison_report', description: 'Write the complete report.html document verbatim to the Host-owned report sink.', parameters: Type.Object({ html: Type.String({ minLength: 1, maxLength: MAX_BYTES }) }), execute: async (params) => writeReport(reportRoot, 'report.html', params) };
}
...
async function writeReport(root: string, name: string, params: unknown): Promise<{ content: string; details: Record<string, string | number> }> {
  const value = params as { html?: unknown };
  if (typeof value.html !== 'string') throw new Error('Report HTML must be text.');
  const path = resolve(root, name);
  await writeAtomic(path, value.html);
  const bytes = Buffer.byteLength(value.html);
  return { content: `Wrote ${bytes} bytes.`, details: { path: name, bytes, sha256: createHash('sha256').update(value.html).digest('hex'), writtenAt: new Date().toISOString() } };
}
