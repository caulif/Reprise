...
export function recoveryAcceptIsExposed(input: {
  ...
  if (input.envelopeStatus === "blocked" || input.envelopeStatus === "insufficient_evidence") return false;
  if (input.match === "current_state_fallback") return false;
  if (input.runnable === "blocked" || input.runnable === "unsupported") return false;
  return input.envelopeStatus === "ready" || input.envelopeStatus === "recovered" || input.envelopeStatus === "partial";
}
