...
/** Report link label for the result page; diagnostic kinds keep the richer T14 wording. */
export function reportLabelOf(presentation: ResultPresentation, locale: Locale): string {
  if (presentation.reportKind !== 'diagnostic') return t(locale, 'resultReport');
  if (presentation.comparisonKind === 'failed') return t(locale, 'comparisonDiagnosticFailed');
  if (presentation.comparisonKind === 'cancelled') return t(locale, 'comparisonDiagnosticCancelled');
  if (presentation.comparisonKind === 'insufficient_evidence') return t(locale, 'comparisonDiagnosticInsufficient');
  return t(locale, 'comparisonDiagnosticUnknown');
}

/** Pill accepts ok|warn|off; map danger down so header and body share one statusTone. */
export function pillToneOf(tone: ResultTone): 'ok' | 'warn' | 'off' {
