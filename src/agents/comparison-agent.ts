...
export const COMPARISON_SYSTEM_PROMPT = [
  ...
  '',
  'The card face is only: the Host title, Host labels, one headline sentence, the shortest contrast that states the difference, optional paired finals, and the Host metrics. One or two sentences are enough when they say what each side delivered and what the user still must do; use a table only when parallel items would tangle, and then at most five data rows. Notes in work/comparison-plan.md may be detailed; the card must stay compressed.',
  '',
...
export const COMPARISON_TURN_PROMPTS = {
  ...
  investigate: [
    ...
    '',
    'For visual tasks, list both sides\' available images by side from media.json, pair them by page or by file role, and record in work/comparison-plan.md: left ref, right ref, whether both are finals of the same kind. If a side is missing, the card will not show images; do not plan a one-sided preview.',
    '',
  ...
  compose: [
    ...
    '- data-agent-zone="key-differences": one contrast that must be non-empty. Prefer one or two sentences when that states what each side delivered and what the user still must do. Use a table only when parallel items would tangle in a sentence; if you use a table, at most five data rows (delivery form, the gap that changes the user\'s decision, optional user follow-up). Do not add process rows (replay SHA, sink range, Controller turn counts, which session library was scanned first). Do not treat two to four difference cards as a skeleton. If you use difference-card, use at most one, and do not repeat the headline. When no comparison is possible, say so explicitly ("cannot be determined") and why.',
    '- data-agent-zone="key-differences": one or two short sentences, or at most three bullets. Prefer pointing to visual-evidence when paired finals exist; do not paste tables here when images are available.',
    '- data-agent-zone="visual-evidence": copy pair-pages with the historical model on the left and the candidate model on the right when paired finals exist; leave empty when there are no paired images. Do not paste full deliverable text here. Do not put a montage next to three page images on the card. If only one side has images, leave this zone empty; one-sided previews are not a comparison.',
  ...
  review: [
    ...
    '- Is the headline still before key-differences?',
    ...
    '- Do cited images exist, belong to the right side, and load? Without paired images, is visual-evidence empty? Is a one-sided image sitting next to "no mounted preview"?',
