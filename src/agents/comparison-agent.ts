...
export const COMPARISON_TURN_PROMPTS = {
  ...
  compose: [
    ...
    '- data-agent-zone="key-differences": one contrast that must be non-empty. Prefer one or two sentences when that states what each side delivered and what the user still must do, or at most three bullets. Prefer pointing to visual-evidence when paired finals exist; do not paste tables here when images are available. Use a table only when parallel items would tangle in a sentence; if you use a table, at most five data rows. Do not add process rows (replay SHA, sink range, Controller turn counts, which session library was scanned first). Do not treat two to four difference cards as a skeleton. If you use difference-card, use at most one, and do not repeat the headline. When no comparison is possible, say so explicitly ("cannot be determined") and why.',
