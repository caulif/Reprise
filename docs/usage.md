...
3. 选择候选产品，再选择该 Pack 的模型目录项。候选目录来自 `ProductRuntime.listCatalog()`，不是 Harness 内部 Pi 模型列表。模型列表 `Enter` 进入确认页，**不会**启动候选。
4. 确认页复述任务、来源/候选产品、请求模型与解析模型、恢复结论与限制。确认页 `Enter` 才启动候选；`b` 与 `Esc` 返回模型列表（不丢弃恢复、不重跑恢复）。受阻的恢复诊断页除外：`Esc` 返回封面。
