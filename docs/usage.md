...
历史对话列表支持方向键、可打印文本筛选、`Ctrl+F` 切换可运行过滤、`Ctrl+N` 加载下一页、`Ctrl+R` 从第一页刷新。核对页没有上下选择项；展开详情后，长核对、准备回看、启动确认和结果详情仍可用方向键或 PageUp/PageDown 阅读，但页脚不常驻显示翻页提示。首页“运行记录”可凭任务标题和状态寻找旧运行。设置页用方向键选择字段，`Enter` 行内编辑或切换，低频字段在“更多设置”；`Ctrl+T` 测试连接，`Ctrl+S` 显式保存，离开未保存草稿时默认继续编辑。
