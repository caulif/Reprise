...
- 只读观察材料是否未被改写。
- 工作副本 HEAD 是否为任务开始前的提交；HEAD 已含历史任务提交，或任务结束后的脏文件仍留在候选可见树（运行依赖目录除外）时，不得发布 `ready`。
