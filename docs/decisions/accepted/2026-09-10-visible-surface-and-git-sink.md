...
- Harness 拥有的树（Recovery staging、published baseline、`prepareRun` 副本）里，每个 Git 仓库的 `origin`/`pushurl` 指向本实验 `environment/git-sinks/{id}` bare sink。sink 目录名为相对路径 SHA-256 前 12 位（根仓 `_root.git`），可读名在 `INDEX.tsv`。建立 bare 用 `init` + `fetch`，Git 打开 `core.longpaths`。候选进程去掉 `GITHUB_TOKEN`/`GH_TOKEN`，并用 sink `gitconfig`（`GIT_CONFIG_GLOBAL` / `NOSYSTEM`）对已记录 origin URL 做 `insteadOf`；不改写 `HOME`/`USERPROFILE`。Comparison 读 `candidate/git-sink-refs.txt`，不把用户 GitHub 当实验远端。
...
- `test/core/git-sink.test.ts`：嵌套仓 `prepareRun` 后 origin 不是用户远端；对 sink `push` 前进；对用户远端 URL `push` 不前进用户 HEAD。`caulif/themes/PaperMod` 的 sink 目录是 12 位哈希而不是路径拼名。
- `npm run check`。
