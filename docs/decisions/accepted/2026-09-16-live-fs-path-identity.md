...
`git cat-file -e HASH^{commit}` 在部分 Windows Git 包装下会丢掉 `^`。shell_exec 只在 Windows 把非零退出当结果返回，POSIX 直接抛错，覆盖率 lane 无法观察 `exitCode`。`runProcess` 与 CandidateRun cleanup 的超时 `unref()` 后，事件循环可在超时触发前结束。

## 决定

活文件系统比较用 `realpath`（`sameLiveFsPath`）。记录路径仍用 `sameFsPath`，8.3 与长名字符串不相等。Git toplevel 与 historical commit 探测走活路径；commit 存在性用 `git cat-file -t`，避免 `^`。TUI 帧比对先把 Users/temp 前缀（含 8.3）收成 `C:\user\...`，再去掉行末 `│` 前的多余空白。`shell_exec` 在所有宿主返回非零退出码。进程超时定时器保持引用，直到该次 `runProcess` / cleanup 结束。
