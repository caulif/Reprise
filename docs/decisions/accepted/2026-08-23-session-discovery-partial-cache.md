...
`partial` 只描述列表摘要不完整，不改变 inspect/import 的完整文件限制，也不承诺可回放。有界读取不消费模型正文或工具输出；不读取凭据、不联网、不启动 Runtime、不落盘索引。旧 Pack 不提供 partial consumer 时行为保持原样；新字段可选，旧消费者忽略它。refresh 只清除对应产品/root 的内存缓存。

## 备选方案

**超限文件一律不进列表。** 用户无法从列表识别过大的真实会话，只能面对无身份的 diagnostic。

**把摘要索引落盘。** 会新增 on-disk 格式和隐私面，超出列表扫描所需，也与“不落盘索引”冲突。

## 影响

- 超限但头部可确认身份的会话可以出现在列表中，并带 `partial: true`。
- 单文件变化不再迫使整棵 root 重读。
- 完整 inspect/import 仍受原限制；partial 项不能被当成可回放。
