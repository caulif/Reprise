...
5. 相同 mutation version、工具名和参数的重复调用属于无信息增益调用，由 Host 拒绝；受控写入或 shell 成功会推进 mutation version，因此写入后的重新读取仍被允许。硬性 `maxToolCalls` 预算继续生效；Host 按 hypothesis 递减剩余搜索预算，并拒绝估计成本超过剩余预算的 probe。
