...
- 只允许当前 bundle origin 与 `data:`/`blob:`；阻断 `file:`、其他 loopback 端口、外网、WebSocket、EventSource、`sendBeacon`、新窗口与下载。未能建立受控加载时返回 `capability_unavailable`，不用宽权限 `file://` 或 `--no-sandbox` 兜底。
...
- `test/application/artifact-renderer.test.ts`：fake 多帧、校验、取消、bundle 路径逃逸；opt-in 真浏览器双帧、HTTP 与 WebSocket 阻断。
