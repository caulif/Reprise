...
- `render_artifact` / `preview_report` 工具工厂在 `comparison-render-tools.ts`，经 `comparison-render-catalog.ts` 适配 B3 catalog（`resolveSource` / `registerDerivedMedia` / `revision`）。`sourceRef` 只接受已注册短引用；历史来源优先映射到 attempt `finals/`（或受控 `history/`），候选来源映射到 candidate snapshot；`media/*` / `review/*` 的 `bundleRoot` 分别为 `attemptRoot/media` 与 `attemptRoot/review`（不得用整个 attempt 树作文档静态根）；渲染前校验 on-disk `contentHash`。`registerDerivedMedia` 始终调用 catalog `registerMedia`（由 catalog 去重并在 emit 失败后重试补发事件），派生键不含 `capturedAt`。`preview_report` 先用与发布相同的 `preparePublishableComparisonHtml` 物化临时 HTML（format-2：仅在 `comparison` / `details` 区内把 `data-media-ref` 写成可加载 `src`）；`report_review` 使用独立 `review-*` 短引用，不得 mint `media-*` 以免污染比较证据 allowlist。
...
- Comparison 工具面挂载真实 `render_artifact` / `preview_report`（`experiment-report.ts` → catalog 适配器）；不再使用 capability-unavailable stub。
...
- `test/application/comparison-render-tools.test.ts`：工具注册、去重 shortRef、preview digest、`review-*` 与 `media-*` 命名空间分离；Host catalog 适配、合法 sourceRef 多帧、未知/逃逸/非法 viewport/取消结构化失败。
