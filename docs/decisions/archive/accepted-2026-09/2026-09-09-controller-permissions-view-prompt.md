...
- 当前 `view.txt` 的可见助手文本只来自最近一次 `runtime.turn_settled` 对应的事件区间，且是该区间内全部公开 `text` 的拼接（段间 `trim` 后用空行连接；thinking / tool_use 不进表面）；整次运行的 `finalMessage` 仍供 Comparison 摘要使用。
