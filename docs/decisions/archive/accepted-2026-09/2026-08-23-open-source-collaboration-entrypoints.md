...
Issue / PR 模板和 `CODEOWNERS` 放在 `.github/`。任务 brief 模板放在 [`docs/plan/task-brief-template.md`](../../../CONTRIBUTING.md)。`npm run verify:docs` 检查这些路径存在，并检查文档或模板含有规定标记；缺少任一路径或必填标记时门禁失败。
