...
- [x] **Phase 2：统一全量项目树**：Codex/Claude 初次 discovery 在未传 cursor 时建立完整轻量目录；TUI 在完整目录上排序、搜索、筛选和项目分组，`m` 不再决定是否发现全部会话，视口只切片绘制；项目外会话和索引状态可见。 未编入 Desktop 的 rollout 保留 `rollout-only/unindexed` 来源证据，同时按计划归入项目外会话，不把 cwd 伪造成正式项目。
