...
| 运行（含恢复） | `/` / Tab / `v` / Ctrl+C / ? | 查找时间线 / 可展开组 / 阅读模式 / 请求取消 / 按键说明；Esc 不取消 |
