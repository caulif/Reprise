...
→ 选择该产品下的项目
→ 选择该产品的历史会话
→ 自动开始恢复并等待结果（已恢复 / 部分恢复 / 缺关键输入，补上后可重跑 / 无法恢复）
...
主时间线不直接倾倒底层 event payload。内部 Agent 的 `agent.tool_*` 按 `payload.role` 分轨：进行中只留一行最新工具 tip（叶名），刚成功的同质探路收成一条 `▸ 阅读证据 · N`（或写入变体），禁止把每个成功探路叶名落成主列永久行；下一段短句到来时 flush 定格为同一折叠。工具 stdout 与上下文 JSON 不进主列，运行页不设全文 overlay；排障打开结果页记录目录。一个实验一条单列连续记录：恢复、模拟用户投递、用户可见回合、结果与可选对照按持久化顺序追加；模拟用户页与对照页不把上一声部过程带进主列。投递消息走 Input 声部，不得画成候选桃色。内部 Agent 子弹为薄荷，候选用桃色。运行主列用左缘两列分声部，正文默认前景；折叠 `▸` 与叶名变暗；失败独立红；`now:*` 此刻行只出现在列尾，不在主列再画一遍；列尾行左动作、右时钟；向上阅读暂停跟随时画 `▼ N`。候选用户可见内容来自已校验的 `candidate.user_view_persisted`；重开只读事件日志，不加载 Pack。未公开 reasoning 与产品私有 runtime payload 不进主列。Comparison 叠在同一时间线：结果段按 `c` 才开对照，跳过则对照为未运行。滚动按条目身份保存阅读位置；向上阅读暂停跟随，Home 到最早可见条，End 或 `l` 跟随最新。滚轮与 ↑↓ 相同：先移选中，贴边时改视口偏移；视口库不得先消费 SGR 64/65 与单击按下。实验内 `/` 只在候选运行与对照过程搜索已投影的可见标题与短文案（含折叠组内标题与尚未展开的记录），不搜索长工具原文或 `original`；恢复与准备检查不进入查找。结果页单击产物短标签与 `o` / `h` / `f` / `t` / `w` 打开同一目标；未命中不打开文件。Enter / Shift+Enter 在命中间移动并展开所在组。`v` 关闭鼠标报告并暂停当前视图重绘，后台仍追加持久化事件；退出后提示暂停期间的新活动条数。查找中的 `v` 写入查询，不进入阅读模式。本地产物链接：路径须为已验证绝对路径；可见文本始终是短标签。支持 OSC 8 时输出 `file:` URI；不支持时仍显示短标签，不改成绝对路径。标签与 URI 去掉控制序列。进程 `exit`、未捕获异常会调用 `tui.stop()` 以恢复终端模式。压缩粒度见[UserVisibleTurn 时间线](../decisions/accepted/2026-09-10-user-visible-turn-timeline.md)。

恢复页标题绑定 `runPhase==='recovery'`（以及准备态 `preparePhase==='check'`），文案是「正在恢复会话」；超过约 30 秒无新进展时顶栏改「仍在恢复会话」，画布不再复述标题。该阶段列尾写恢复，空画布不得写成候选正在写回复。选完模型进入候选运行，标题是「候选运行中 · {候选产品}」或「正在启动 {候选产品}」；顶栏任务短句用候选产品显示名，不用来源会话产品。用户终态为无法恢复或没有 accept 时，禁止启动隔离候选，标题不得声称已准备隔离对照，原因留一句人话（校验失败时附代码）：变更为 0 时说明没有观察到隔离工作区变更，有变更才强调工作区校验未通过；禁止只显示 `provider_validation_failed`。`partial` 且校验通过的 preview 必须暴露 accept。运行栏显示当前阶段、最近 Runtime 事件和重连次数；超过 120 秒无新事件时提示可 Ctrl+C。封面、列表、核对、恢复、选择、运行、对照过程与结果的可滚动区都接鼠标滚轮。候选失败时 `termination.code` 保持 `failed.runtime`，类别与脱敏摘要写在 `failure`；上游暂时不可用由用户重新启动候选，不自动重试。
...
只有 `ControllerDecision.send.message` 会发送给 Target。同一句投递只画一张 Input 卡：`input.submitted` 与 Pack `prompt` 若正文相同则合并进已有气泡，不叠第二段。有 Input 卡不再画 `Decision: SEND`。Controller 的可见短句属于主列；工具参数、`rationale` 全文和证据引用不进默认列。Delivery 状态必须来自 Runtime 协议，不能依据界面是否继续活动推测；`unknown`、拒绝和超时应明确显示。
...
1. **Comparison 摘要**：Host 投影终止、已有硬数和报告入口；有对照时可选显示信封 `headline`。不把 Controller `done/satisfied` 理由当成对照结论。跳过对照时对照为未运行，不写两边都如何；结果页仍列出报告 / 历史终稿 / 候选终稿。token/cost 未采集时显示 `not recorded`。不解析 `report.html`，不给统一质量总分。
2. **本地 HTML 报告**：提供详细并排比较。第一版是一次性生成的静态本地文件，不实现完整 Web 应用或第二套控制面。
3. **原始详情**：Harness 自有的持久化 trace、decision 和可用 telemetry 可通过本地入口打开；token/cost 未采集时显示 `not recorded`。记录目录与候选隔离副本（`environment/runs/{runId}`）是排查入口，`t` / `w` 打开；cleanup 不删除隔离副本中的交付物。第一版不承诺内嵌通用 artifact 预览，专有 artifact 只保证 metadata 和安全打开入口。
...
| 会话（项目） | ↑↓ Enter 输入 Ctrl+F Ctrl+N Ctrl+R Esc | 选择、打开、筛选、只看可跑、更多、刷新、回封面 |
| 会话（会话） | ↑↓ Enter 输入 Ctrl+N Ctrl+R Backspace Esc | 选择、核对、筛选、更多、刷新、回项目、返回 |
...
| 运行（恢复） | Ctrl+C | 请求取消；只读观看 |
| 运行（候选 / 对照过程） | Ctrl+C | 请求取消；Esc 不取消；只读观看 |
| 运行（查找中） | Enter / S-Enter / Esc / Ctrl+C | 下一处 / 上一处 / 清除查找 / 请求取消 |
| 运行（阅读模式） | `v` / Esc / Ctrl+C | 恢复画面 / 请求取消 |
| 结果 | o / h / f / t / w / c / Esc | 打开报告 / 历史终稿 / 候选终稿 / 记录 / 副本 / 开始对照 / 回封面；主路径短标签可点 |
| 错误 | Enter / b / Esc | 返回 |

运行页是只读观看面：不向目标 Runtime 打字，续问由 Controller 产生。帮助层按当前页面显示。运行阶段由共享 Runtime 生命周期事件投影，适用于所有 Pack。

结果段叠在同一条记录底部，三行人话对应任务判断、运行终止、清理；跳过对照写「对照未运行」，仍显示报告 / 历史终稿 / 候选终稿行（无文件则空位）。主路径短标签打开报告与两侧终稿；记录与隔离副本为排查入口（支持 OSC 8 时用 `file:` 链接）。比较失败显示失败类别，入口标为 Diagnostic。配置页密钥使用完整遮罩，会话列表摘要排除共享识别规则认定的注入指令，原始会话仍用于检查和回放。手填源目录只在历史 cwd 缺失时出现，封面 `/` 进不了该页。
